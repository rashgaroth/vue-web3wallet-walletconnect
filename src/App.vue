  ...
  <div id="app" class="bg-main min-h-screen p-10">
    <div class="flex flex-col space-y-5 items-center">
      ...
      <LegacySessionProposalModal 
        :openModal="wcData.openModal && wcData.modalType === 'LegacySessionProposalModal'" 
        :wcData="wcData" 
      ...
      <LegacySessionSignTypedDataModal 
        :openModal="wcData.openModal && wcData.modalType === 'LegacySessionSignTypedDataModal'"
        :wcData="wcData"
        ...
        :signData="wcData?.data?.legacyCallRequestEvent?.params?.[1] || 'Unknown'"
...
import { signTypedData } from './gql/mutations';
...
export default {
  ...
  async mounted() {
    ...
    if (signClient && signClient?.session && signClient.session.values.length === 0) {
      this.dappConnected = false
    }
  },
  methods: {
    ...
    onLegacySessionProposalRequest(type) {
      if (type === constants.legacySessionProposalType.APPROVE) {
        ...
        this.walletConnectController.legacySignClient.rejectSession(getSdkError("USER_REJECTED_METHODS"))
        this.wcData.modalType = ''
        this.wcData.openModal = false
      }
    },
    ...
    openNotification(position = null, color, { title = "info", text = "nothing" }) {
      ...
      if (result === constants.legacySessionProposalType.APPROVE) {
        ...
        try {
          ...
          if (signature.data.res) {
            ...
            this.wcData.modalType = ''
            this.wcData.openModal = false
            this.wcData.data.legacyCallRequestEvent = { id: 0, method: '', params: [] }
          }
        } catch (error) {
          this.openNotification('top-left', 'danger', {
            title: "Error",
            text: "Transaction rejected with error: " + error.message || "Unknown error"
          })
          this.loading = false
          const reqId = this.wcData.data.legacyCallRequestEvent.id
          this.walletConnectController.legacySignClient.rejectRequest({
